...
import copy, math, utils, os, pdb, gc
...
def omniquant(lm, args, dataloader, act_scales, act_shifts, logger=None, **kwargs):
    logger.info("Starting ...")

    # move embedding layer and first layer to target device
    model, dev = lm.model, lm.device
    ...
    for i in range(len(layers)):
        ...
        if args.epochs > 0:
            ...
            # create optimizer
            optimizer = torch.optim.AdamW([{"params": qlayer.let_parameters(use_shift), "lr": args.let_lr}, {"params": qlayer.lwc_parameters(), "lr":args.lwc_lr}], weight_decay=args.wd)
